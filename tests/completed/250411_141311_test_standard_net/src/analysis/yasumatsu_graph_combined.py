...
def yasumatsu_graph(bpath, nsp):
    ...
    try:
        ...
        for k,stepsize in enumerate([1,5,25]): 

            tstart = 100
            label = 't=\SI{' + '%d' %((synee_a['t'][tstart+stepsize] - synee_a['t'][tstart])/second) + '}{s}'


            syn_a = synee_a['a'][tstart:,:]
            syn_a = syn_a[np.arange(0,1000-tstart+stepsize,stepsize),:]
            ...
            axes[k,0].set_ylim(-1*np.max(np.abs(stat))/5,np.max(np.abs(stat))/5)
            axes[k,1].set_ylim(-1*np.max(np.abs(stat))/5,np.max(np.abs(stat))/5)
            axes[k,2].set_ylim(-1*np.max(np.abs(stat))/5,np.max(np.abs(stat))/5)
    ...
    directory = "figures/yasumatsu_graph_combined"
